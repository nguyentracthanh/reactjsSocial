...
export default function Products() {
  ...
  const getCatalogID = () => {
    ...
    Axios
      .get(`https://graph.facebook.com/v7.0/${IDcatalog}/product_sets?access_token=${userAccessToken}`
      )
      .then(
        (res) => {
  ...
  const getProduct = () => {
    for (var i = 0; productSetDetail.length; i++) {
      ...
      Axios
        .get(`https://graph.facebook.com/v7.0/${productSetDetail[i]}/products?access_token=${userAccessToken}`
        )
        .then(
          (res) => {
  ...
  return (
    <React.Fragment>
      <button  id="buttonProductlist" onClick={() => Onclicked()}> GET productset</button>
      {productSetDetail.map(productSetDetailItem =>
        <menu toggle={false}  key={productSetDetailItem.id}>
          {productSetDetailItem.name}
          {nameProduct.map(nameItem =>
              <li key={nameItem.id}>
                <nav>
                  {nameItem.name}
                  <br />
